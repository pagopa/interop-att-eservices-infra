  ...
  data = {
    ...
    "parsers.conf" = <<-EOF
      [PARSER]
        Name        slf4j
        Format      regex
        Regex       (?<t>[^ ]+) (?<stream>stdout|stderr) (?<logtag>P|F) (?<time>[^ ]+)\s+(?<level>\S+) \[(?<traceId>[^\]]*)\] - \[UID=(?<clientId>[^\]]*)\] \[OID=(?<organizationId>[^\]]*)\] \[CID=(?<correlationId>[^\]]*)\] (?<message>.*)$
        Time_Key    time
...
resource "aws_iam_policy" "log" {
  name        = "AttEksFargateLoggingAccessPolicy"
  path        = "/"
